...
def _test_get_location():
    ...
    assert out.admin1.unique()[0] == 'California', f'get_outbreak_data not returning correct data for {location}'
...
def _test_get_multiple_locations():
    ...
    out = outbreak_data.get_outbreak_data('covid19/query',
                                          ...
                                          collect_all=True)
    assert len(out.admin1.unique()) == len(location), f'get_outbreak_data returning incorrect number of locations, ' \
...
def _test_cases_multiple_location():
    ...
    assert len(out.admin1.unique()) == len(location), f'cases_by_location returning an incorrect number of locations,' \
                                          f' expected {len(location)}'
    location_names = sorted(['California', 'New York', 'Texas'])
    output_names = sorted(out.admin1.unique())
    for i in range(len(location_names)):
